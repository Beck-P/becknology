...
(function () {
  ...
  // ─── Per-game arcade cabinets ─────────────────────────────────────────
  // Each cabinet wraps a single tile and previews its real game's identity.
  // The shared skeleton is: floor light pool → body → outline → screen
  // (with pulse + scrolling content + scanlines) → marquee → joystick + buttons → feet.

  // Asteroids — pure white-on-black vector arcade
  function drawAsteroidsCabinet(ctx, x, y, ts, time, col, row) {
    time = time || 0; col = col || 0; row = row || 0;
    var u = ts / 16;
    var phase = (time / 700) + (col * 5 + row * 11);
    var pulse = 0.7 + Math.sin(phase) * 0.3;
    // White floor halo
    ctx.save();
    ctx.globalCompositeOperation = 'screen';
    ctx.globalAlpha = pulse * 0.45;
    var castGrad = ctx.createRadialGradient(x + ts * 0.5, y + ts * 1.05, 0, x + ts * 0.5, y + ts * 1.05, ts * 1.0);
    castGrad.addColorStop(0, 'rgba(255,255,255,0.55)');
    castGrad.addColorStop(1, 'transparent');
    ctx.fillStyle = castGrad;
    ctx.fillRect(x - ts * 0.4, y + ts * 0.4, ts * 1.8, ts * 1.0);
    ctx.restore();
    ...
    ctx.fillRect(x + 2*u, y + u, u, ts - 2*u);
    ctx.fillRect(x + ts - 3*u, y + u, u, ts - 2*u);
    // Screen — black with white wireframes
    var sx = x + 3*u, sy = y + 3*u, sw = ts - 6*u, sh = Math.floor(ts * 0.35);
    ctx.fillStyle = '#000';
    ctx.fillRect(sx, sy, sw, sh);
    // Tiny ship triangle and asteroid pixels (drift across)
    ctx.globalAlpha = pulse;
    ctx.fillStyle = '#fff';
    var shipX = sx + ((time / 60) % sw);
    ctx.fillRect(shipX, sy + Math.floor(sh * 0.5), Math.max(1, u), Math.max(1, u));
    ctx.fillRect((sx + (time / 80) % sw + sw * 0.4) % (sx + sw - u) || sx + 2, sy + Math.floor(sh * 0.25), Math.max(1, u), Math.max(1, u));
    ctx.fillRect(sx + Math.floor(sw * 0.65), sy + Math.floor(sh * 0.7), Math.max(1, u), Math.max(1, u));
    ctx.globalAlpha = 1;
    // Scanlines
    ...
    // Marquee — thin white bar
    ctx.fillStyle = '#1a1a1a';
    ctx.fillRect(x + 3*u, y + Math.floor(ts * 0.55), ts - 6*u, Math.floor(ts * 0.15));
    ctx.fillStyle = '#fff';
    ctx.globalAlpha = 0.85 + Math.sin(time / 300) * 0.15;
    ctx.fillRect(x + 4*u, y + Math.floor(ts * 0.58), ts - 8*u, Math.max(1, u));
    ctx.globalAlpha = 1;
    // Joystick + buttons
    ctx.fillStyle = '#aaa';
  ...
  // Helicopter — synthwave magenta + cyan with sun on screen
  function drawHelicopterCabinet(ctx, x, y, ts, time, col, row) {
    time = time || 0; col = col || 0; row = row || 0;
    var u = ts / 16;
    var phase = (time / 600) + (col * 5 + row * 11);
    var pulse = 0.75 + Math.sin(phase) * 0.25;
    // Pink/cyan floor halo
    ctx.save();
    ctx.globalCompositeOperation = 'screen';
    ctx.globalAlpha = pulse * 0.55;
    var castGrad = ctx.createRadialGradient(x + ts * 0.5, y + ts * 1.05, 0, x + ts * 0.5, y + ts * 1.05, ts * 1.1);
    castGrad.addColorStop(0, 'rgba(255,57,192,0.7)');
    castGrad.addColorStop(0.6, 'rgba(0,245,255,0.3)');
    castGrad.addColorStop(1, 'transparent');
    ctx.fillStyle = castGrad;
    ctx.fillRect(x - ts * 0.4, y + ts * 0.4, ts * 1.8, ts * 1.1);
    ctx.restore();
    // Body — deep magenta
    ...
    ctx.fillRect(x + 2*u, y + u, u, ts - 2*u);
    ctx.fillRect(x + ts - 3*u, y + u, u, ts - 2*u);
    // Screen — pink to cyan sunset gradient
    var sx = x + 3*u, sy = y + 3*u, sw = ts - 6*u, sh = Math.floor(ts * 0.35);
    var screenGrad = ctx.createLinearGradient(0, sy, 0, sy + sh);
    screenGrad.addColorStop(0, '#5a0e80');
    screenGrad.addColorStop(0.4, '#ff39c0');
    screenGrad.addColorStop(0.65, '#ff8a3a');
    screenGrad.addColorStop(0.7, '#0a0420');
    screenGrad.addColorStop(1, '#0a0420');
    ctx.fillStyle = screenGrad;
    ctx.fillRect(sx, sy, sw, sh);
    // Sun pixel
    ctx.globalAlpha = pulse;
    ctx.fillStyle = '#ffe75e';
    ctx.fillRect(sx + Math.floor(sw * 0.6), sy + Math.floor(sh * 0.5), Math.max(1, 2*u), Math.max(1, u));
    // Cyan grid lines (perspective)
    ctx.fillStyle = '#00f5ff';
    ctx.fillRect(sx, sy + Math.floor(sh * 0.85), sw, Math.max(1, u * 0.5));
    ctx.globalAlpha = 1;
    // Scanlines
    ctx.fillStyle = 'rgba(0,0,0,0.3)';
    for (var sl = 0; sl < sh; sl += 2*u) {
  ...
  // Breakout — candy rainbow stripes
  function drawBreakoutCabinet(ctx, x, y, ts, time, col, row) {
    time = time || 0; col = col || 0; row = row || 0;
    var u = ts / 16;
    var phase = (time / 600) + (col * 5 + row * 11);
    var pulse = 0.75 + Math.sin(phase) * 0.25;
    ...
    var castGrad = ctx.createRadialGradient(x + ts * 0.5, y + ts * 1.05, 0, x + ts * 0.5, y + ts * 1.05, ts * 1.0);
    castGrad.addColorStop(0, 'rgba(255,142,200,0.7)');
    castGrad.addColorStop(0.5, 'rgba(255,214,74,0.3)');
    castGrad.addColorStop(1, 'transparent');
    ctx.fillStyle = castGrad;
    ctx.fillRect(x - ts * 0.4, y + ts * 0.4, ts * 1.8, ts * 1.0);
    ctx.restore();
    // Body — cream/white
    ctx.fillStyle = '#f0d8e0';
    ctx.fillRect(x + 2*u, y + u, ts - 4*u, ts - 2*u);
    ...
    ctx.fillRect(x + 2*u, y + u, u, ts - 2*u);
    ctx.fillRect(x + ts - 3*u, y + u, u, ts - 2*u);
    // Screen — pastel rainbow stripes (mini bricks)
    var sx = x + 3*u, sy = y + 3*u, sw = ts - 6*u, sh = Math.floor(ts * 0.35);
    var brickColors = ['#ff6ec1', '#ff9354', '#ffd64a', '#6ed46a', '#5dc0e0', '#b78cff'];
    ctx.fillStyle = '#1a0f1f';
    ctx.fillRect(sx, sy, sw, sh);
    var stripeH = Math.max(1, Math.floor(sh / 6));
    ctx.globalAlpha = pulse;
    for (var bi = 0; bi < 6; bi++) {
      ctx.fillStyle = brickColors[bi];
      ctx.fillRect(sx, sy + bi * stripeH, sw, stripeH);
    }
    ctx.globalAlpha = 1;
    // Tiny ball pixel bouncing
    var bp = (time / 100) % sw;
    ctx.fillStyle = '#fff';
    ctx.fillRect(sx + bp, sy + sh - 2*u, Math.max(1, u), Math.max(1, u));
    // Scanlines
    ctx.fillStyle = 'rgba(0,0,0,0.2)';
    for (var sl = 0; sl < sh; sl += 2*u) {
      ctx.fillRect(sx, sy + sl, sw, Math.max(1, u * 0.5));
    }
    // Marquee — rainbow gradient stripe
    var mx = x + 3*u, my = y + Math.floor(ts * 0.55), mw = ts - 6*u, mh = Math.floor(ts * 0.15);
    var mGrad = ctx.createLinearGradient(mx, 0, mx + mw, 0);
    mGrad.addColorStop(0, '#ff6ec1'); mGrad.addColorStop(0.33, '#ffd64a');
    mGrad.addColorStop(0.66, '#5dc0e0'); mGrad.addColorStop(1, '#b78cff');
    ctx.fillStyle = mGrad;
    ctx.fillRect(mx, my, mw, mh);
    // Joystick + buttons (pink + yellow)
    ctx.fillStyle = '#ff39c0';
    ctx.fillRect(x + Math.floor(ts * 0.35), y + Math.floor(ts * 0.55), 2*u, 3*u);
    ctx.fillStyle = '#ffd64a';
    ctx.fillRect(x + Math.floor(ts * 0.55), y + Math.floor(ts * 0.6), u, u);
    ctx.fillStyle = '#ff6ec1';
    ctx.fillRect(x + Math.floor(ts * 0.7), y + Math.floor(ts * 0.6), u, u);
    // Feet
    ctx.fillStyle = '#d8b8c0';
    ctx.fillRect(x + 3*u, y + ts - 3*u, 2*u, 3*u);
  ...
  // Space Invaders — deep navy with mini multicolor pixel-aliens
  function drawSpaceInvadersCabinet(ctx, x, y, ts, time, col, row) {
    time = time || 0; col = col || 0; row = row || 0;
    var u = ts / 16;
    var phase = (time / 600) + (col * 5 + row * 11);
    var pulse = 0.75 + Math.sin(phase) * 0.25;
    ...
    ctx.globalAlpha = pulse * 0.5;
    var castGrad = ctx.createRadialGradient(x + ts * 0.5, y + ts * 1.05, 0, x + ts * 0.5, y + ts * 1.05, ts * 1.0);
    castGrad.addColorStop(0, 'rgba(0,245,255,0.7)');
    castGrad.addColorStop(0.5, 'rgba(255,57,192,0.3)');
    castGrad.addColorStop(1, 'transparent');
    ctx.fillStyle = castGrad;
    ctx.fillRect(x - ts * 0.4, y + ts * 0.4, ts * 1.8, ts * 1.0);
    ctx.restore();
    ...
    ctx.fillRect(x + 2*u, y + u, u, ts - 2*u);
    ctx.fillRect(x + ts - 3*u, y + u, u, ts - 2*u);
    // Screen — black with rainbow alien rows
    var sx = x + 3*u, sy = y + 3*u, sw = ts - 6*u, sh = Math.floor(ts * 0.35);
    ctx.fillStyle = '#02030f';
    ctx.fillRect(sx, sy, sw, sh);
    // 3 rows of tiny pixel aliens (yellow / cyan / pink)
    var alienColors = ['#ffd64a', '#00f5ff', '#ff39c0'];
    var rowH = Math.max(1, Math.floor(sh / 4));
    var pixelSize = Math.max(1, Math.floor(u * 0.8));
    ctx.globalAlpha = pulse;
    for (var ar = 0; ar < 3; ar++) {
      ctx.fillStyle = alienColors[ar];
      var ay = sy + 1 + ar * rowH;
      // 3 alien pixel pairs across the row
      for (var ac = 0; ac < 4; ac++) {
        var ax = sx + 1 + Math.floor(ac * (sw - 2) / 3);
        ctx.fillRect(ax, ay, pixelSize, pixelSize);
      }
    }
    ctx.globalAlpha = 1;
    // Tiny player ship at bottom
    ctx.fillStyle = '#00f5ff';
    ctx.fillRect(sx + Math.floor(sw * 0.5) - u, sy + sh - 2*u, Math.max(1, 2*u), Math.max(1, u));
    // Scanlines
  ...
  // Missile Command — olive military with radar sweep on screen
  function drawMissileCommandCabinet(ctx, x, y, ts, time, col, row) {
    time = time || 0; col = col || 0; row = row || 0;
    var u = ts / 16;
    var phase = (time / 600) + (col * 5 + row * 11);
    var pulse = 0.75 + Math.sin(phase) * 0.25;
    ...
    ctx.globalAlpha = pulse * 0.5;
    var castGrad = ctx.createRadialGradient(x + ts * 0.5, y + ts * 1.05, 0, x + ts * 0.5, y + ts * 1.05, ts * 1.0);
    castGrad.addColorStop(0, 'rgba(255,170,58,0.7)');
    castGrad.addColorStop(0.5, 'rgba(183,255,122,0.25)');
    castGrad.addColorStop(1, 'transparent');
    ctx.fillStyle = castGrad;
    ctx.fillRect(x - ts * 0.4, y + ts * 0.4, ts * 1.8, ts * 1.0);
    ctx.restore();
    // Body — olive military
    ctx.fillStyle = '#383d28';
    ctx.fillRect(x + 2*u, y + u, ts - 4*u, ts - 2*u);
    ctx.fillStyle = '#1a1d10';
    ctx.fillRect(x + 3*u, y + u, ts - 6*u, 2*u);
    // Amber outline
    ...
    ctx.fillRect(x + 2*u, y + u, u, ts - 2*u);
    ctx.fillRect(x + ts - 3*u, y + u, u, ts - 2*u);
    // Screen — dark green with concentric radar rings + sweep
    var sx = x + 3*u, sy = y + 3*u, sw = ts - 6*u, sh = Math.floor(ts * 0.35);
    ctx.fillStyle = '#060d06';
    ctx.fillRect(sx, sy, sw, sh);
    // concentric circles (drawn as squares for chunky pixel feel)
    var cxr = sx + sw * 0.5, cyr = sy + sh * 0.5;
    ctx.globalAlpha = pulse * 0.7;
    ctx.strokeStyle = '#b7ff7a';
    ctx.lineWidth = Math.max(1, u * 0.5);
    for (var r = 1; r < 4; r++) {
      ctx.beginPath();
      ctx.arc(cxr, cyr, r * u, 0, Math.PI * 2);
      ctx.stroke();
    }
    // sweep line
    var sweep = (time / 600) % (Math.PI * 2);
    ctx.strokeStyle = '#b7ff7a';
    ctx.lineWidth = Math.max(1, u * 0.5);
    ctx.beginPath();
    ctx.moveTo(cxr, cyr);
    ctx.lineTo(cxr + Math.cos(sweep) * sw * 0.4, cyr + Math.sin(sweep) * sh * 0.4);
    ctx.stroke();
    ctx.globalAlpha = 1;
    // Mini cities at bottom (cyan dots)
    ctx.fillStyle = '#5dc0e0';
    ctx.fillRect(sx + Math.floor(sw * 0.2), sy + sh - 2*u, Math.max(1, u), Math.max(1, u));
    ctx.fillRect(sx + Math.floor(sw * 0.5), sy + sh - 2*u, Math.max(1, u), Math.max(1, u));
    ctx.fillRect(sx + Math.floor(sw * 0.8), sy + sh - 2*u, Math.max(1, u), Math.max(1, u));
    // Scanlines
    ctx.fillStyle = 'rgba(0,0,0,0.3)';
    for (var sl = 0; sl < sh; sl += 2*u) {
      ctx.fillRect(sx, sy + sl, sw, Math.max(1, u * 0.5));
    ...
    ctx.fillRect(mx, my, mw, mh);
    ctx.fillStyle = '#ff8c28';
    ctx.globalAlpha = 0.7 + Math.sin(time / 250) * 0.3;
    ctx.fillRect(mx + 2*u, my + Math.floor(mh * 0.35), mw - 4*u, Math.max(1, u));
    ctx.globalAlpha = 1;
    // Joystick + red FIRE oval
    ctx.fillStyle = '#888';
    ctx.fillRect(x + Math.floor(ts * 0.35), y + Math.floor(ts * 0.55), 2*u, 3*u);
    ctx.fillStyle = '#ff3a3a';
    ctx.fillRect(x + Math.floor(ts * 0.55), y + Math.floor(ts * 0.6), 2*u, u);
    // Feet
    ctx.fillStyle = '#1a1d10';
    ctx.fillRect(x + 3*u, y + ts - 3*u, 2*u, 3*u);
    ctx.fillRect(x + ts - 5*u, y + ts - 3*u, 2*u, 3*u);
  ...
  BridgeWorld.registerTileset('arcadia', {
    ...
    41: drawAsteroidsCabinet,
    42: drawHelicopterCabinet,
    43: drawBreakoutCabinet,
    44: drawSpaceInvadersCabinet,
    45: drawMissileCommandCabinet
  });
